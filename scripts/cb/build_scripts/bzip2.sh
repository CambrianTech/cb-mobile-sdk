...
function build_bzip2_selector() {

    download_dependency "bzip2"
    ...
    case $1 in
        ...
        osx | linux)
            ...
            ;;
        *)
            cb_logger "Don't know how to build bzip2 for $1"
            exit 1
        ;;
    esac
...
function build_bzip2_native() {

    echo "Building for arch ${PLATFORM}"

    ### path setup
    BUILD_DIR="$CB/third-party/bzip2/src/bzip2/build"
    INSTALL_DIR="$CB/prebuilts/bzip2/${PLATFORM}"
...
function build_bzip2_android() {

    if [ $# -eq 0 ]; then
        build_bzip2_android --arch 32
        build_bzip2_android --arch 64
        build_bzip2_android --arch x86
        build_bzip2_android --arch x86_64
        exit
    fi
    ...
    ### path setup
    BUILD_DIR="$CB/third-party/bzip2/src/bzip2/build"
    INSTALL_DIR=$CB/prebuilts/bzip2/android/${ABI}
...
function build_bzip2_ios() {
    ...
    SRC_DIR="${CB}/third-party/bzip2/src/bzip2"
    BUILD_DIR="${SRC_DIR}/build"

    rm -Rdf $BUILD_DIR/*
    mkdir -p $BUILD_DIR

    INSTALL_DIR=$CB/prebuilts/bzip2
    ...
    for SDK in "${IPHONE_SDKS[@]}"; do
        ...
        for ARCH in "${ARCHS[@]}"; do
            ...
            THIS_LIB="${THIS_BUILD_DIR}/${BUILD_TYPE}-${SDK}/bzip2.a"
    ...
    lipo -create ${BINARY_PATHS} -o ${FINAL_BINARY_PATH}/bzip2.a
}