package com.cambrian.cbar.sensor.orientationProvider;
        ...
        if (event.sensor.getType() == Sensor.TYPE_ROTATION_VECTOR) {
            ...
            if (timestamp != 0) {
                ...
                if (panicCounter > PANIC_THRESHOLD) {
                    Timber.tag("Rotation Vector").d("Panic counter is bigger than threshold; this indicates a Gyroscope failure. Panic reset is imminent.");

                    if (gyroscopeRotationVelocity < 3) {
                        Timber.d("Performing Panic-reset. Resetting orientation to rotation-vector value.");
                        ...
                        Timber.tag("Rotation Vector")
                              .d(String.format("Panic reset delayed due to ongoing motion (user is still shaking the device). Gyroscope Velocity: %.2f > 3", gyroscopeRotationVelocity));
                    }
                }
            }
