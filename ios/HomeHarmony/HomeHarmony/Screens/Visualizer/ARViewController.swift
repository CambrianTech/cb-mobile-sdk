...
class ARViewController: UIViewController, BottomSheetAction, ToolDelegate, CBRemodelingViewDelegate, SendItemDelegate {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if callLayout {
            ...
            if self.model!.hasSentItem() {
                ...
                self.itemSelected(self, item: self.model!.sentItem!)
                self.model?.sentItem = nil // reset the sent item so it does
    ...
    func setupAssetVC() {
        ...
        if let assetsVC = assetsVC {
            ...
            assetsVC.setup(image: self.model!.image)
        }
    ...
    func enableLiveView(_ isEnabled:Bool) {
        ...
        if(model!.incomingSharedProject!) {
            isLiveView = false
            model!.incomingSharedProject = false  // reset the incoming shared project so it does
    ...
    func showTutorialBubble(_ tutorial: Tutorial) {
        ...
        if let message = message {
            message.isHidden = model!.hasShownTutorial(tutorial)
            // Remove tutorial after a certain delay
            if !message.isHidden {
                DispatchQueue.background(delay: 3.0, completion: {
                    message.isHidden = true
                    self.model!.setHasShownTutorial(tutorial)
                })
            }
        }
    }
    ...
    func showTutorial(_ tutorial: Tutorial) {
        ...
        if tutorial == .Rotation {
            if (model!.selectedItem!.isFloor() == false || model!.hasCreatedFloor) {
                return
            }
            hud.indicatorView = JGProgressHUDImageIndicatorView(image: UIImage(named: "ic_rotate")!)
        } else if tutorial == .Detail {
            if (self.isLiveView || model!.selectedItem!.isPaint() || !model!.hasPainted) {
                return
            }
        }
    ...
    func setRawImage(_ rawImage: UIImage) {
        self.model?.rawImage = rawImage
    }
    
    func setSentItem(item: BrandItem) {
        self.model?.sentItem = item
    }
    ...
    func saveProject(_ path: String) {
    ...
    func historyChanged(_ asset: CBAugmentedAsset, change: CBUndoChange, forward: Bool) {
        self.model?.historyChanged(asset, change: change, forward: forward)
    ...
    @objc func backPressed() {
        ...
        if self.model!.hasImagePath() || self.model!.hasRawImage() {
    ...
    func displayEditable() {
        displayMessage("You are now able to edit this shared design")
    }
