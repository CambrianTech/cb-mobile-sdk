                        ...
                        isBGR:(BOOL)bgr;
{
    ...
    @autoreleasepool {
        size_t totalPixels = imageSize.width * imageSize.height * channels;
        NSData *data = [NSData dataWithBytes:bytes length:totalPixels];
...
+ (CIImage *) CoreImageForCVMat:(const cv::Mat&)image isBGR:(BOOL)isBGR {
    cv::Mat img = image;
    if (image.channels() == 1) {
        cv::cvtColor(image, img, cv::COLOR_GRAY2BGRA);
    }
    else if (image.channels() == 3) {
        cv::cvtColor(image, img, cv::COLOR_RGB2RGBA);
    }
    return [self ciimageWithBytes:img.data imageSize:CGSizeMake(img.cols, img.rows) channels:img.channels() isBGR:isBGR];
}
...
+ (cv::Mat)CVMatForCGImage:(CGImageRef)image;
{
    ...
    cv::Mat cvMat=cv::Mat::zeros(rows, cols, CV_8UC4); // 8 bits per component, 4 channels
...
+ (CVPixelBufferRef) pixelBufferFromCVMat:(const cv::Mat &) image options:(NSDictionary *)options {
    CIImage * ciImage = [self CoreImageForCVMat:image isBGR:NO];
...
+ (CGAffineTransform)imageTransformForRotation:(int)degrees
    ...
    bool isDiagonal = (degrees / 90) % 2;
    
    CGAffineTransform trans = CGAffineTransformMakeRotation(DEGREES_TO_RADIANS(degrees));
    
    float imageAspectRatio = isDiagonal ? imageSize.height / imageSize.width : imageSize.width / imageSize.height;
    float outputAspectRatio = viewportSize.width / viewportSize.height;
    
    float sx = imageAspectRatio > outputAspectRatio ? 1.0 : outputAspectRatio / imageAspectRatio;
    float sy = imageAspectRatio > outputAspectRatio ? imageAspectRatio / outputAspectRatio : 1.0;
    
    const float min = fmin(sx, sy);
    const float max = fmax(sx, sy);
    ...
    //0.5 × (vw/sx - vw) / vw
    //0.5 × (vh/sy - vh) / vh
    float tx = 0.5 * (viewportSize.width / sx - viewportSize.width) / viewportSize.width;
    float ty = 0.5 * (viewportSize.height / sy - viewportSize.height) / viewportSize.height;
    ...
    if (tx != 0.0f || ty != 0.0f) {
        trans = CGAffineTransformTranslate(trans, tx, ty);
    }
    
    //CBLog("sx=%f, sy=%f, tx=%f, ty=%f, rotated=%d, imageSize:%fx%f", sx, sy, tx, ty, isDiagonal, imageSize.width, imageSize.height);
