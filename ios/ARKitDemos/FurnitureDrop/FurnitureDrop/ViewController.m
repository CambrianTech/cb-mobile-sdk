...
- (void)setupPhysics {
  ...
  bottomNode.position = SCNVector3Make(0, planeDepth, 0);
...
- (void)geometryConfigFrom: (UILongPressGestureRecognizer *)recognizer {
  ...
  } else if ([parentNode isKindOfClass:[SCNReferenceNode class]]) {
...
- (void)explode:(ARHitTestResult *)hitResult {
  ...
  for(SCNNode *assetNode in self.assets) {
    // The distance between the explosion and the geometry
    SCNVector3 distance = SCNVector3Make(
                                          assetNode.worldPosition.x - position.x,
                                          assetNode.worldPosition.y - position.y,
                                          assetNode.worldPosition.z - position.z
                                          );
    ...
    [[assetNode.childNodes firstObject].physicsBody applyForce:distance atPosition:SCNVector3Make(0.05, 0.05, 0.05) impulse:YES];
  }
}
...
- (void)insertCube:(ARHitTestResult *)hitResult {
  ...
  SCNVector3 position = SCNVector3Make(
    ...
    NSString *filePath = [[NSBundle mainBundle] pathForResource:@"model" ofType:@"dae" inDirectory:@"art.scnassets"];
    
    NSURL *referenceURL = [NSURL fileURLWithPath:filePath];
    SCNReferenceNode *node = [SCNReferenceNode referenceNodeWithURL:referenceURL];
    
    node.castsShadow = true;
    
    float height = 0.45;
    SCNBox *cube = [SCNBox boxWithWidth:0.6 height:height length:0.6 chamferRadius:0];
    SCNMaterial *noMaterial = [SCNMaterial new];
    noMaterial.diffuse.contents = [UIColor colorWithWhite:0.5 alpha:0.0];
    cube.materials = @[noMaterial];
    
    
    node.pivot = SCNMatrix4MakeTranslation(0, height / 2, 0);
    //SCNNode *geom = [SCNNode nodeWithGeometry:cube];
    node.geometry = cube;
