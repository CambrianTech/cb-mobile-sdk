...
- (std::string)getPassedAssetID {
    return std::string([_assetID UTF8String]);
}
...
void CBARAssetCallback::_touchesCommon(const cbpipe::TouchPoint &touch, CBAugmentedAsset **asset, CGPoint *touchPoint) {
    ...
    *touchPoint = CGPointMake(touch.screenOrigin.x * touch.srcSize.width / scale,
                              touch.screenOrigin.y * touch.srcSize.height / scale);
